//************************************************************
//
// blocking.c
//
// Gefan Zhang
//
//*************************************************************
// Modified by Fajar Purnama
// Only receive TOSH_DATA_LENGTH of data usually 28 bytes

#include <stdio.h>
#include <fcntl.h>
#include <unistd.h>
#include <sys/ioctl.h>
#include <tosmac.h>

void msg_init(TOS_Msg* pMsg)
{
   pMsg->length = 0;
   pMsg->fcfhi = 0;
   pMsg->fcflo = 0;
   pMsg->dsn = 0;
   pMsg->destpan = 0;
   pMsg->addr = 0;
   pMsg->type = 0;
   pMsg->group = 0;
   memset(pMsg->data, 0, TOSH_DATA_LENGTH);
   pMsg->strength = 0;
   pMsg->crc = 0;
   pMsg->lqi = 0;
   pMsg->ack = 0;
   pMsg->time = 0;
}
//--------------------- main -------------------------------

int main(int argc, const char *argv[])
{
  if(argv[1]==NULL){
   printf("Usage: recv_small_file [file to be created]\n");
  return 1;
  }
  int tosmac_dev;
  int file;
  TOS_Msg recv_pkt;
  TOS_Msg send_pkt;
  
  // open as blocking mode
  tosmac_dev = open(TOSMAC_DEVICE, O_RDWR);
  if (tosmac_dev < 0)
  {
    fprintf(stderr, "Open error: %s\n", TOSMAC_DEVICE);
    return 1;
  }

  printf("User read from driver:\n");
    
  //read(tosmac_dev, &recv_pkt, sizeof(TOS_Msg));// != sizeof(TOS_Msg);
  read(tosmac_dev, &recv_pkt, recv_pkt.length);  
  printf("Receiving %d bytes of data\n", recv_pkt.length);
  creat(argv[1], O_RDWR);
  file = open(argv[1], O_RDWR);
  write(file, recv_pkt.data, recv_pkt.length);
  // close device
  close (tosmac_dev);
  close(file);

  return 0;
}
