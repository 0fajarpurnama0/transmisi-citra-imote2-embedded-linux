...
int main(int argc, const char *argv[]) {
 ...
 while(1){
  i = lseek(file,0,SEEK_END); // seek to end of the file, so the data received will not overwrite (for discrete transmission)
  memcpy(send_pkt.data,"!GO!",4); // !GO! message to inform transmitter to send
  write(tosmac_dev, (TOS_Msg*)&send_pkt, sizeof(TOS_Msg)); // sending !GO! as ready to reaceive next packet (ACK)
